...
class RegisterMedicoPago(View):
    def get(self, request, pk):
        registrations = get_object_or_404(RegisterMedico, pk=pk)
        registrations.pagMes    = "Pagor"
        registrations.save()
        return redirect("list_med")
    
class RegisterPacientesPago(View):
    def get(self, request, pk):
        registrations = get_object_or_404(RegisterPacientes, pk=pk)
        registrations.pagMes  = "Pagor"
        registrations.save()
        return redirect("list_pac")
...
class ConsultaCreateView(CreateView):
    model = RegisterAgenda
    fields = ['nome_medico', 'cpf_paciente', 'data_hora','obs_consulta']
